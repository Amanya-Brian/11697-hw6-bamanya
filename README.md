...
2. **Out of memory when loading Llama**
   - Solution: Use smaller model or run on CPU
   - Edit `llama_generator.py` to use "meta-llama/Llama-3.2-1B-Instruct"

3. **Slow Dense Retriever**
   - Dense retriever computes embeddings for all documents
   - First run will be slower
   - Consider caching embeddings for large corpora
...
1. **Use GPU for Llama**: Set device in generator initialization
2. **Batch Processing**: Increase `--batch_size` for faster processing
3. **Top-K Tuning**: Experiment with different `--top_k` values (3-5 is typical)
4. **Cache Embeddings**: For dense retriever, save embeddings for reuse

## Notes

- The BM25 retriever is fully open-weight and requires no API calls
- Dense retriever uses OpenAI embeddings API
- GPT generator uses OpenAI API
- Llama generator runs locally and is fully open-weight
- All outputs are saved in TSV format as required by the assignment

## Next Steps (Part 4 - Evaluation)

After generating predictions, you'll need to:
1. Implement evaluation metrics (LLM-as-judge + your choice)
2. Run evaluation on all prediction files
3. Compare results across different configurations
4. Analyze which combination works best for your football dataset